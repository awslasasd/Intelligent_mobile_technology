...
% Any questions please contact: zjuyinhuan@gmail.com
    ...
    for i=1 : nSteps
        ...
        jF = jacobF(xEkf, u);
        PPred = jF * PxEkf * jF' + convQ;

        % Update
        jH = jacobH(xPred);
        K = PPred * jH' / (jH * PPred * jH' + convR);
        xEkf = xPred + K * (z - doObservation(z, xPred));
        PxEkf = (eye(size(K * jH)) - K * jH) * PPred;
        % -----------------------------
...
function x = doMotion(x, u)
    global dt;
    % 运动学模型：x = [x, y, yaw]', u = [vx, vy, w]'
    x(1) = x(1) + u(1) * dt * cos(x(3)) - u(2) * dt * sin(x(3));
    x(2) = x(2) + u(1) * dt * sin(x(3)) + u(2) * dt * cos(x(3));
    x(3) = x(3) + u(3) * dt;
end
...
function jF = jacobF(x, u)
    global dt;
    % 计算运动模型的雅各比矩阵
    jF = [1, 0, -u(1) * dt * sin(x(3)) - u(2) * dt * cos(x(3));
          0, 1,  u(1) * dt * cos(x(3)) - u(2) * dt * sin(x(3));
          0, 0,  1];
end
...
function []=finalPlot(estimation)
    ...
    % calculate error
    ekf_Error = mean(sqrt(sum((estimation.xEkf - estimation.xTruth).^2, 2)));
    odom_Error = mean(sqrt(sum((estimation.xOdom - estimation.xTruth).^2, 2)));

    % 输出误差
    disp('定位误差统计：');
    disp(['Odom平均误差: ', num2str(odom_Error), ' 米']);
    disp(['EKF平均误差: ', num2str(ekf_Error), ' 米']);
